...
import com.i1314i.ourproject.service.RecruitService;
...
public class RecruitController {
    ...
    public ResultMap insert(@RequestBody Recruit recruit) {
        boolean flag = recruitService.insert(recruit);
        if (flag)
    ...
    public ResultMap deleteById(@PathVariable("ids") String ids) {
        ...
        if (ids.contains("-")) {
            ...
            flag = recruitService.deleteAllByPrimaryKey(list);
        } else {
            flag = recruitService.deleteOneByPrimaryKey(ids);
        }
        if (flag)
            return resultMap.start().msg("success").code("200");
        else
    ...
    public ResultMap updateByPrimaryKey(@RequestBody Recruit recruit) {
        boolean flag = false;
        flag = recruitService.updateByPrimaryKey(recruit);
        if (flag)
            return resultMap.start().msg("success").code("200");
        return resultMap.start().msg("fail").code("500");
    }
    ...
    public ResultMap getDataByPrimaryKey(@PathVariable("id") String id) {
        Recruit recruit= recruitService.getDataByPrimaryKey(id);
        if (recruit != null)
            return resultMap.start().msg("success").code("200").data(recruit);
        else
    ...
    public void downDemo(HttpServletResponse response) throws Exception {
        ...
        String[] keys = {"编号", "单位名称","单位地址", "招聘岗位", "招聘要求", "联系方式","行业类型", "发布时间"};
        String[] col = {"zhaogongId", "name", "address", "station", "demand", "telphone", "type","fabutiem"};
        List<Recruit> list = new ArrayList<>();
        JavaPoiUtil.createWorkBooks(Recruit.class, list, col, keys).write(response.getOutputStream());
    }
