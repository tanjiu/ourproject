...
import com.i1314i.ourproject.service.SoilTypeDistributionService;
...
/**
 * @author 叹久
 * @create 2018-10-04 21:08
 */
@Controller
@RequestMapping("/sdc")
public class SoilTypeDistributionController {
    @Autowired
    SoilTypeDistributionService soilTypeDistributionService;
    @Autowired
    ResultMap resultMap;

    private Logger logger = LoggerFactory.getLogger(SoilTypeDistributionController.class);
    /**
     * 分页获取数据
     */
    @ResponseBody
    ...
    public ResultMap getDataByPage(@RequestParam(value  ="pn",defaultValue = "1") Integer pn){
        ...
        List<SoilTypeDistribution> list=soilTypeDistributionService.getAll();
     ...
     * 插入一条数据
     *
     * @param soilTypeDistribution
     * @return
     */
    //由于ajax传过来的是一个字符串，所以要使用@RequestBody注解进行转换为po对象
    @ResponseBody
    @RequestMapping(value = "/insertapc", method = RequestMethod.POST)
    public ResultMap insert(@RequestBody SoilTypeDistribution soilTypeDistribution) {
        boolean flag = soilTypeDistributionService.insert(soilTypeDistribution);
        if (flag)
    ...
    public ResultMap deleteById(@PathVariable("ids") String ids) {
        ...
        if (ids.contains("-")) {
            ...
            flag = soilTypeDistributionService.deleteAllByPrimaryKey(list);
        } else {
            flag = soilTypeDistributionService.deleteOneByPrimaryKey(ids);
        }
        if (flag)
            return resultMap.start().msg("success").code("200");
        else
    ...
    public ResultMap updateByPrimaryKey(@RequestBody SoilTypeDistribution soilTypeDistribution) {
        boolean flag = false;
        flag = soilTypeDistributionService.updateByPrimaryKey(soilTypeDistribution);
        if (flag)
            return resultMap.start().msg("success").code("200");
        return resultMap.start().msg("fail").code("500");
    }
    ...
    public ResultMap getDataByPrimaryKey(@PathVariable("id") String id) {
        SoilTypeDistribution soilTypeDistribution = soilTypeDistributionService.getDataByPrimaryKey(id);
        if (soilTypeDistribution != null)
            return resultMap.start().msg("success").code("200").data(soilTypeDistribution);
        else
    ...
    public void downDemo(HttpServletResponse response) throws Exception {
        ...
        String[] keys = {"编号", "类型","分布"};
        String[] col = {"leixingId", "type", "region"};
        List<SoilTypeDistribution> list = new ArrayList<>();
        JavaPoiUtil.createWorkBooks(SoilTypeDistribution.class, list, col, keys).write(response.getOutputStream());
    }
    ...
    public void down(HttpServletResponse response) throws Exception {
        JavaPoiUtil.setDownType(response, "土地类型分布数据文件", 2003);
        List<SoilTypeDistribution> list = soilTypeDistributionService.getAll();
        String[] keys = {"编号", "类型","分布"};
        String[] col = {"leixingId", "type", "region"};
        JavaPoiUtil.createWorkBooks(SoilTypeDistribution.class, list, col, keys).write(response.getOutputStream());
