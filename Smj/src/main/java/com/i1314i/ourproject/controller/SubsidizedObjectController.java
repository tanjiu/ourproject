...
import com.i1314i.ourproject.service.SubsidizedObjectService;
...
/**
 * @author 叹久
 * @create 2018-10-06 11:29
 */
@Controller
@RequestMapping("/soc")
public class SubsidizedObjectController {
    @Autowired
    SubsidizedObjectService subsidizedObjectService;
    @Autowired
    ResultMap resultMap;

    private Logger logger = LoggerFactory.getLogger(SubsidizedObjectController.class);
    /**
     * 分页获取数据
     */
    @ResponseBody
    ...
    public ResultMap getDataByPage(@RequestParam(value = "pn", defaultValue = "1") Integer pn) {
        ...
        List<SubsidizedObject> list = subsidizedObjectService.getAll();
     ...
     * 插入一条数据
     *
     * @param subsidizedObject
     * @return
     */
    //由于ajax传过来的是一个字符串，所以要使用@RequestBody注解进行转换为po对象
    @ResponseBody
    @RequestMapping(value = "/insertapc", method = RequestMethod.POST)
    public ResultMap insert(@RequestBody SubsidizedObject subsidizedObject) {
        boolean flag = subsidizedObjectService.insert(subsidizedObject);
        if (flag)
    ...
    public ResultMap deleteById(@PathVariable("ids") String ids) {
        ...
        if (ids.contains("-")) {
            ...
            flag = subsidizedObjectService.deleteAllByPrimaryKey(list);
        } else {
            flag = subsidizedObjectService.deleteOneByPrimaryKey(Integer.parseInt(ids));
        }
        if (flag)
            return resultMap.start().msg("success").code("200");
        else
    ...
    public ResultMap updateByPrimaryKey(@RequestBody SubsidizedObject subsidizedObject) {
        boolean flag = false;
        flag = subsidizedObjectService.updateByPrimaryKey(subsidizedObject);
        if (flag)
            return resultMap.start().msg("success").code("200");
        return resultMap.start().msg("fail").code("500");
    }
    ...
    public ResultMap getDataByPrimaryKey(@PathVariable("id") int id) {
        SubsidizedObject subsidizedObject = subsidizedObjectService.getDataByPrimaryKey(id);
        if (subsidizedObject != null)
            return resultMap.start().msg("success").code("200").data(subsidizedObject);
        else
            return resultMap.start().msg("NoData").code("404");
    }
    ...
    public ResultMap getSouSuo(@RequestBody SouSuo souSuo) {
        List<SubsidizedObject> list = subsidizedObjectService.getSouSuo(souSuo);
        if (list != null)
            return resultMap.start().msg("success").code("200").data(list);
        else
            return resultMap.start().msg("NoData").code("404");
     ...
     */
    @ResponseBody
    @RequestMapping("/getDataByMoHu/{mingcheng}")
    public ResultMap getDataByMoHu(@PathVariable("mingcheng") String mingcheng){
        List<SubsidizedObject> list = subsidizedObjectService.getDataByMoHu(mingcheng);
        if (list != null)
            return resultMap.start().msg("success").code("200").data(list);
        else
            return resultMap.start().msg("NoData").code("404");
    ...
    public ResultMap getDataByDanWei(@PathVariable("danwei") String danwei){
        List<SubsidizedObject> list = subsidizedObjectService.getDataByDanWei(danwei);
        if (list != null)
            return resultMap.start().msg("success").code("200").data(list);
        else
            return resultMap.start().msg("NoData").code("404");
    ...
    public void downDemo(HttpServletResponse response) throws Exception {
        ...
        String[] keys = {"补贴名称", "发放部门","发放形式", "发放时间", "申请流程"};
        String[] col = {"butieMingcheng", "fafangBumen", "fafangXingshi", "fafangShijian", "shenqing"};
        List<SubsidizedObject> list = new ArrayList<>();
        JavaPoiUtil.createWorkBooks(SubsidizedObject.class, list, col, keys).write(response.getOutputStream());
    }
    ...
    public void down(HttpServletResponse response) throws Exception {
        JavaPoiUtil.setDownType(response, "补贴对象数据文件", 2003);
        List<SubsidizedObject> list = subsidizedObjectService.getAll();
        String[] keys = {"补贴名称", "发放部门","发放形式", "发放时间", "申请流程"};
        String[] col = {"butieMingcheng", "fafangBumen", "fafangXingshi", "fafangShijian", "shenqing"};
        JavaPoiUtil.createWorkBooks(SubsidizedObject.class, list, col, keys).write(response.getOutputStream());
