...
import com.i1314i.ourproject.service.ProductPriceService;
...
public class ProductPriceController {
    ...
    public ResultMap insert(@RequestBody ProductPrice productPrice) {
        boolean flag = productPriceService.insert(productPrice);
        if (flag)
    ...
    public ResultMap deleteById(@PathVariable("ids") String ids) {
        ...
        if (ids.contains("-")) {
            ...
            flag = productPriceService.deleteAllByPrimaryKey(list);
        } else {
            flag = productPriceService.deleteOneByPrimaryKey(Integer.parseInt(ids));
        }
        if (flag)
            return resultMap.start().msg("success").code("200");
        else
    ...
    public ResultMap updateByPrimaryKey(@RequestBody ProductPrice productPrice) {
        boolean flag = false;
        flag = productPriceService.updateByPrimaryKey(productPrice);
        if (flag)
            return resultMap.start().msg("success").code("200");
        return resultMap.start().msg("fail").code("500");
    }
    ...
    public ResultMap getDataByPrimaryKey(@PathVariable("id") int id) {
        ProductPrice productPrice = productPriceService.getDataByPrimaryKey(id);
        if (productPrice != null)
            return resultMap.start().msg("success").code("200").data(productPrice);
        else
     ...
     */
    @ResponseBody
    @RequestMapping("/getByMarket/{shichang}")
    public ResultMap getByFk(@PathVariable("shichang") String shichang){
        List<ProductPrice> list=productPriceService.getByMarket(shichang);
        if (list.size()!=0)
            return resultMap.start().msg("success").code("200").data(list);
        else
            return resultMap.start().msg("NoData").code("404");
    ...
    public ResultMap getByName(@PathVariable("name") String name){
        List<ProductPrice> list=productPriceService.getByName(name);
        if (list.size()!=0)
            return resultMap.start().msg("success").code("200").data(list);
        else
            return resultMap.start().msg("NoData").code("404");
    ...
    public void downDemo(HttpServletResponse response) throws Exception {
        ...
        String[] keys = {"编号", "产品名称","类别", "单价", "单位", "市场"};
        String[] col = {"jiageId", "name", "category", "monavalent", "unit", "market"};
        List<ProductPrice> list = new ArrayList<>();
        JavaPoiUtil.createWorkBooks(ProductPrice.class, list, col, keys).write(response.getOutputStream());
    }
