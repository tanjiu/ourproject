...
import com.i1314i.ourproject.service.ProductionEnterprisesService;
...
/**
 * @author 叹久
 * @create 2018-10-04 20:11
 */
@Controller
@RequestMapping("/pec")
public class ProductionEnterprisesController {
    @Autowired
    ProductionEnterprisesService productionEnterprisesService;
    @Autowired
    ResultMap resultMap;

    private Logger logger = LoggerFactory.getLogger(ProductionEnterprisesController.class);
    /**
     * 分页获取数据
     */
    @ResponseBody
    ...
    public ResultMap getDataByPage(@RequestParam(value  ="pn",defaultValue = "1") Integer pn){
        ...
        List<ProductionEnterprises> list=productionEnterprisesService.getAll();
     ...
     * 插入一条数据
     *
     * @param productionEnterprises
     * @return
     */
    //由于ajax传过来的是一个字符串，所以要使用@RequestBody注解进行转换为po对象
    @ResponseBody
    @RequestMapping(value = "/insertapc", method = RequestMethod.POST)
    public ResultMap insert(@RequestBody ProductionEnterprises productionEnterprises) {
        boolean flag = productionEnterprisesService.insert(productionEnterprises);
        if (flag)
    ...
    public ResultMap deleteById(@PathVariable("ids") String ids) {
        ...
        if (ids.contains("-")) {
            ...
            flag = productionEnterprisesService.deleteAllByPrimaryKey(list);
        } else {
            flag = productionEnterprisesService.deleteOneByPrimaryKey(Integer.parseInt(ids));
        }
        if (flag)
            return resultMap.start().msg("success").code("200");
        else
    ...
    public ResultMap updateByPrimaryKey(@RequestBody ProductionEnterprises productionEnterprises) {
        boolean flag = false;
        flag = productionEnterprisesService.updateByPrimaryKey(productionEnterprises);
        if (flag)
            return resultMap.start().msg("success").code("200");
        return resultMap.start().msg("fail").code("500");
    }
    ...
    public ResultMap getDataByPrimaryKey(@PathVariable("id") int id) {
        ProductionEnterprises productionEnterprises = productionEnterprisesService.getDataByPrimaryKey(id);
        if (productionEnterprises != null)
            return resultMap.start().msg("success").code("200").data(productionEnterprises);
        else
    ...
    public void downDemo(HttpServletResponse response) throws Exception {
        ...
        String[] keys = {"单位名称", "单位地址","产品种类", "生产经营方式", "法人代表", "法人代表手机号", "联系人", "联系人手机号", "x坐标", "y坐标"};
        String[] col = {"name", "address", "type", "jingyingFangshi", "farenTelphone", "legalRepresentative", "lianxiren", "lianxirenTel", "x", "y"};
        List<ProductionEnterprises> list = new ArrayList<>();
        JavaPoiUtil.createWorkBooks(ProductionEnterprises.class, list, col, keys).write(response.getOutputStream());
    }
    ...
    public void down(HttpServletResponse response) throws Exception {
        JavaPoiUtil.setDownType(response, "农资生产企业数据文件", 2003);
        List<ProductionEnterprises> list = productionEnterprisesService.getAll();
        String[] keys = {"单位名称", "单位地址","产品种类", "生产经营方式", "法人代表", "法人代表手机号", "联系人", "联系人手机号", "x坐标", "y坐标"};
        String[] col = {"name", "address", "type", "jingyingFangshi", "farenTelphone", "legalRepresentative", "lianxiren", "lianxirenTel", "x", "y"};
        JavaPoiUtil.createWorkBooks(ProductionEnterprises.class, list, col, keys).write(response.getOutputStream());
