...
public class VerifyCodeUtils {
    ...
    public static Validate outputVerifyImageBase64(int w, int h, String verifyCode) throws IOException{
        ByteArrayOutputStream bs = null;
        Validate validate=new Validate();
        try{
            bs = new ByteArrayOutputStream();
            outputImage(w, h, bs, verifyCode);
            String imgsrc = Base64.byteArrayToBase64(bs.toByteArray());
            validate.setBase64Str(imgsrc);
        }catch (Exception e){
            logger.info("验证码生成失败（base64）");
        }finally {
            bs.close();
        }
        return validate;
